...
export const updateOrderStatus = async (req, reply) => {
  try {
    ...
    if(order.deliveryPartner.toString() !== userId){
        return reply.status(400).send({ message: "You are not authorized to update this order" });
    }
...
export const getOrders = async (req, reply) => {
  try {
    ...
    const orders = await Order.find(query).populate("customer").populate("customer branch items.item deliveryPartner");
